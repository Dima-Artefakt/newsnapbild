...
// Маппинг для правильных названий файлов (для путей к картинкам)
...
// Данные для текстовых карточек (заголовки и описания)
...
export const UseCases = () => {
  // По умолчанию открываем "Видео" (индекс 2), как у вас было в коде
  ...
  // Получаем данные только для ТЕКУЩЕЙ активной вкладки
  ...
  return (
    <section id="use-cases" className="dds-tabs dds-reveal">
      {/* Радио-кнопки для поддержки CSS-переключения если оно есть в ваших стилях */}
      ...
      <div className="dds-tabs-inner">
          ...
          <div className="dds-tabs-group" role="tablist">
            {tabs.map((tab, i) => (
              <label
                ...
                onClick={() => {
                  setActiveTab(tab)
                  setActiveIndex(0) // Сбрасываем индекс на 0 при смене вкладки
                  setProgress(0)
                }}
              >
                {tab}
        ...
        <div className="dds-tabs-body">
          {/* Блок с текстовыми карточками — теперь рендерит ТОЛЬКО текущий таб */}
