...
export const FAQ = () => {
  ...
  // Распределение как на скриншоте:
  // Левая колонка: вопросы 1, 2, 3, 4
  // Правая колонка: вопросы 5, 6, 7, 8
  const leftCol = faqData.filter(item => item.id <= 4)
  const rightCol = faqData.filter(item => item.id > 4)
            ...
            <div key={item.id} className="dds-accordion-item">
              ...
              <label className="dds-accordion-head" htmlFor={`dds-faq-${item.id}`}>
                ...
                <span className="dds-accordion-icon">
                  <img src="/plus-icon.svg" alt="+" width="24" height="24" />
                </span>
              </label>
